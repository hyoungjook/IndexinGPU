...
./bin/masstree_bench num-keys=100000000 min-key-length=1 max-key-length=4 min-key-length=1 max-key-length=4
./bin/fixedhashtable_bench num-keys=100000000 min-key-length=1 max-key-length=4 min-key-length=1 max-key-length=4
./bin/extendhashtable_bench num-keys=100000000 min-key-length=1 max-key-length=4 min-key-length=1 max-key-length=4
```

## Measurement results in Azure NC24ads A100 v4 VM:

By using `gallatin_allocator`, compared to our `simple_slab_allocator`, we observe average **3.0x** slowdown on insert throughputs and **1.4x** slowdown on delete throughputs.
GPUChainHT at half-warp tiles showed up to **3.7x** slowdown.
Note that read operations (find, successor) do not allocate/deallocate memory blocks, so the throughputs have no big impact.
Also, GPUCuckooHT has minimal impact as the main hash array is pre-allocated; Gallatin is only used for suffix node alloc/deallocations.
