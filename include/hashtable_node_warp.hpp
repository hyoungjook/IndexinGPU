...
struct hashtable_node_warp {
  ...
  DEVICE_QUALIFIER void update(int location, const value_type& value, uint32_t keystate) {
    ...
    if (tile_.thread_rank() == keystate_bits_lane_) {
      ...
      lane_elem_ |= ((keystate & keystate_mask_) << key_location_x2);
    }
  }

  DEVICE_QUALIFIER void merge(hashtable_node_warp<tile_type, allocator_type>& next_node) {
    ...
    if (tile_.thread_rank() == keystate_bits_lane_) {
      auto num_keys_x2 = 2 * num_keys();
      uint32_t num_keys_x2_mask = ((1u << num_keys_x2) - 1);
      lane_elem_ &= num_keys_x2_mask;
      lane_elem_ |= ((next_node.lane_elem_ << num_keys_x2) & ~num_keys_x2_mask);
    }
    ...
    if (tile_.thread_rank() == next_ptr_lane_) {
      ...
      next_node.lane_elem_ = node_index_;
    }
