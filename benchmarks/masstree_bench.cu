...
void bench_masstree(thrust::device_vector<key_slice_type>& d_keys,
                    ...
                    thrust::device_vector<value_type>& d_scan_results,
                    thrust::device_vector<size_type>& d_scan_result_lengths,
                    uint32_t num_keys,
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    tree.template scan<false, false, reuse_root>(
      ...
      max_value_length, d_scan_result_lengths.data().get(), nullptr, nullptr);
...
int main(int argc, char** argv) {
  ...
  auto d_scan_results   = thrust::device_vector<value_type>(num_keys * max_counts_per_query * max_value_length, invalid_value);
  auto d_scan_result_lengths = thrust::device_vector<size_type>(num_keys * max_counts_per_query, 0);
