...
struct cpu_folly_adapter {
  ...
  using index_type = folly::ConcurrentHashMap<key_type, value_type, key_hash, key_equal>;
  ...
  value_type find(const key_slice_type* key, size_type key_length, [[maybe_unused]] unsigned thread_idx) {
    auto iter = index_->find(key_type{key, key_length});
    if (iter != index_->cend()) {
      return iter->second;
    }
 ...
 private:
  ...
  struct configs {
    ...
    configs(std::vector<std::string>& arguments) {
      ...
      FORALL_ARGUMENTS_CPU_FOLLY(PARSE_ARGUMENTS)
    ...
    void print() const {
      ...
      FORALL_ARGUMENTS_CPU_FOLLY(PRINT_ARGUMENTS)
      #undef PRINT_ARGUMENTS
    }
  };
  #undef FORALL_ARGUMENTS_CPU_FOLLY
